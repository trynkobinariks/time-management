...
function parseDate(
  ...
  if (language === 'uk-UA') {
    ...
    if (lowerText.includes('вчора') || lowerText.includes('вчера')) {
    ...
    const ukOrdinalMatch = text.match(
      /(\d+)\s+(січня|лютого|березня|квітня|травня|червня|липня|серпня|вересня|жовтня|листопада|грудня)/i,
    );
    if (ukOrdinalMatch) {
    ...
    const enOrdinalMatch = text.match(
...
function simpleParse(
  ...
  try {
    console.log(
      `Using simpleParse with language: ${language}, text: "${text}"`,
    );
    const lowerText = text.toLowerCase();
    ...
    if (hoursMatch) {
      ...
      if (numMatch) {
        ...
        hours = parseFloat(hourStr);
        console.log(
          `No explicit hours mentioned, using number found in text: ${hours}`,
        );
      } else {
    ...
    if (!projectName && projects.length > 0) {
      ...
      console.log(
        `No project found in text, using first project: ${projectName}`,
      );
    } else {
    ...
    if (language === 'uk-UA') {
      description = text
        ...
        .replace(
          /(\d+(?:[.,]\d+)?)\s*(годин|година|години|год|годину|г\.)/gi,
          '',
        )
    ...
    try {
      ...
      if (language === 'uk-UA') {
        prompt = `
          ...
          - project_name: має відповідати одному з цих проєктів: ${projectNames}
      ...
      if (language === 'uk-UA') {
        ...
        if (lowerCaseText.includes('today')) {
          ...
          console.log(
            "Explicit 'yesterday' mentioned, setting date to yesterday",
          );
          parsedData.date = format(subDays(new Date(), 1), 'yyyy-MM-dd');
        } else {
          ...
          if (!isNaN(dateObj.getTime())) {
            ...
            console.log(
              'Invalid date from OpenAI, falling back to simple parser',
            );
      ...
      const projectMatch = projects.find(
        p => p.name.toLowerCase() === parsedData.project_name.toLowerCase(),
      );

      if (!projectMatch) {
        throw new Error(
          `Project "${parsedData.project_name}" not found in your projects`,
        );
      }
      ...
      console.error(
        'OpenAI parsing failed, using fallback parser:',
        openAIError,
      );
      const parsedData = simpleParse(text, projects, language);
      ...
      if (!parsedData) {
        return NextResponse.json(
          {
            error:
              'Failed to parse speech with both OpenAI and fallback method',
          },
          { status: 422 },
        );
      }
